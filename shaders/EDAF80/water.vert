...
{
    return 0.5* sharpness * frequency * amplitude * pow(sin((position.x * direction.x + position.y * direction.y) * frequency + phase * time) * 0.5 + 0.5, sharpness-1) * cos((position.x * direction.x + position.y * direction.y)*frequency+time*phase);
}
...
void main()
{
    ...
    displaced_vertex.y += wave(vertex.xz, vec2(-1.0, 0.0), 1.0, 0.2, 0.5,2.0, elapsed_time);
    displaced_vertex.y += wave(vertex.xz, vec2(-0.70, 0.7), 0.5 , 0.4,1.3, 2.0, elapsed_time);
    ...
    vs_out.normal = vec3(derivative(vertex.xz, vec2(-1.0, 0.0), 1.0,  0.2, 0.5,2.0, elapsed_time)*-1,1,derivative(vertex.xz, vec2(-1.0, 0.0), 1.0,  0.2, 0.5,2.0, elapsed_time)*0) ;  
    vs_out.normal += vec3(derivative(vertex.xz, vec2(-0.70, 0.7), 0.5 , 0.4,1.3, 2.0, elapsed_time)*-0.7,0,derivative(vertex.xz, vec2(-0.70, 0.7), 0.5 , 0.4,1.3, 2.0, elapsed_time)*0.7);  
    //vs_out.normal *= vec3(normal_model_to_world);
    
    tangents = vec3(1,derivative(vertex.xz, vec2(-1.0, 0.0), 1.0,  0.2, 0.5,2.0, elapsed_time)*-1 +derivative(vertex.xz, vec2(-0.70, 0.7), 0.5 , 0.4,1.3, 2.0, elapsed_time)*0,0);
    binormals = vec3(0,derivative(vertex.xz, vec2(-1.0, 0.0), 1.0,  0.2, 0.5,2.0, elapsed_time)*-0.7 + derivative(vertex.xz, vec2(-0.70, 0.7), 0.5 , 0.4,1.3, 2.0, elapsed_time)*0.7,1);
