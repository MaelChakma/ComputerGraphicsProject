...
{
    auto const vertice_count = (longitude_split_count + 1) * (latitude_split_count + 1);
    ...
    for (unsigned int i = 0u; i < longitude_split_count + 1; ++i) {
        ...
        for (unsigned int j = 0u; j < latitude_split_count + 1; ++j) {
            ...
            // texture coordinates
            texcoords[index] = glm::vec3(static_cast<float>(j) / (static_cast<float>(latitude_split_count)),
                static_cast<float>(i) / (static_cast<float>(longitude_split_count)),
                0.0f);
    ...
    // create index array
    auto index_sets = std::vector<glm::uvec3>(2u * vertice_count);
    ...
    for (unsigned int i = 0u; i < longitude_split_count+1; ++i)
    {
        for (unsigned int j = 0u; j < latitude_split_count+1; ++j)
        {
            index_sets[index] = glm::uvec3(longitude_split_count * (i + 0u) + (j + 0u),
				longitude_split_count * (i + 0u) + (j + 1u),
				longitude_split_count * (i + 1u) + (j + 1u));
            ++index;

            index_sets[index] = glm::uvec3(longitude_split_count * (i + 0u) + (j + 0u),
				longitude_split_count * (i + 1u) + (j + 1u),
				longitude_split_count * (i + 1u) + (j + 0u));
            ++index;
        }
    }
